...
class PatientRecord:
    def __init__(self, name, uid, author, date):
    ...
    def calculate_hash(self):
        hash_data = str(self.timestamp) + self.name + str(self.uid) + str(self.author) + self.date
...
def add_record():
    ...
    record = PatientRecord(name, uid, author, date)
...
def get_history():
    ...
    for block in blockchain:
        if block.uid == uid:
            ...
            continue    
    if len(history)>=1:
        return render_template('propriete_record.html', all_records=history)
    else:
        return 'Record not found.'
    
    

# returning dateing pauthor
@app.route('/')
def index():
